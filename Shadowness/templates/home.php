...
function display_home($info) {
    ...
    if (db_exists(DB_PHOTOS) && db_exists(DB_PHOTO_ALBUMS)) {
        ...
        FROM ".DB_PHOTOS." p
        LEFT JOIN ".DB_USERS." u on p.photo_user = u.user_id
        ...
        if (dbrows($result) > 0) {
            ...
            while ($data = dbarray($result)) {
                $data['thumb_image'] = ($data['photo_thumb1'] && file_exists(INFUSIONS.'gallery/albums/'.(SAFEMODE ? "album_".$data['album_id'] : "")."/thumbs/".$data['photo_thumb1'])) ?
                    INFUSIONS.'gallery/albums/album_'.$data['album_id'].'/thumbs/'.$data['photo_thumb1'] : IMAGES.'imagenotfound.jpg';
