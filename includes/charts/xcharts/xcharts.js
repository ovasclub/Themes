...
(function () {
    ...
    function time(extents, bounds) {
        return d3.time.scale()
            .domain(_.map(extents, function (d) {
                return new Date(d);
            }))
            .range(bounds);
    }
    ...
    function _getExtents(options, data, xType, yType) {
        ...
        extents = {
            x: d3.extent(nData, function (d) {
                return d.x;
            }),
            y: d3.extent(nData, function (d) {
                return d.y;
            })
        };
        ...
        _.each([xType, yType], function (type, i) {
            var axis = (i) ? 'y' : 'x',
                extended;
            extents[axis] = d3.extent(nData, function (d) {
                return d[axis];
            });
            if (type === 'ordinal') {
                return;
    ...
    _.defaults(xChart.prototype, {
        ...
        _drawAxes: function () {
            ...
            var self = this,
                ...
                zeroLine = d3.svg.line().x(function (d) {
                    return d;
                }),
                zLine,
                zLinePath,
